...
export class FetchFoxCrawl implements INodeType {
...
async function executeCrawlerPattern(ex: IExecuteFunctions): Promise <INodeExecutionData[][]> {
	...
	console.log('executeCrawlerPattern', d);
	...
	const items = await getJobResults(ex, jobId);
	console.log('results', items);

	return [ex.helpers.returnJsonArray(items)];
}

async function getJobResults(ex: IExecuteFunctions, jobId: string): Promise<any> {
